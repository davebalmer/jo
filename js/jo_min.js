...
if(joGesture)
joGesture.load();joLog("Jo",this.version,"loaded for",this.platform,"environment.");this.loadEvent.fire();},attachViewClasses:function(){joCollect.view={};joCollect.view.joview=joView;for(var p in window){var o=window[p];if(typeof o==='function'&&o.prototype&&typeof o.prototype.tagName!=='undefined'&&o.prototype instanceof joView){var tag=o.prototype.tagName.toUpperCase();var key=joCollect.view;if(o.prototype.type){if(!key[tag])
...
return this.preference[key];}};joCollect={loaded:0,view:{},get:function(parent){if(!this.loaded++)
jo.attachViewClasses();var parent=joDOM.get(parent);var ui={};var setContainer=joView.setContainer;var draw=joView.draw;parse(parent);joView.setContainer=setContainer;joView.draw=draw;function parse(node){if(!node)
...
function newview(node,args){var tag=node.tagName;var view=node;if(joCollect.view[tag]){if(args instanceof Array&&args.length){if(args.length==1)
args=args[0];}
if(args instanceof Text)
args=node.nodeData;if(!args)
args=node.value||node.checked||node.innerText||node.innerHTML;joView.setContainer=function(){this.container=node;return this;};var o=(node.type)?joCollect.view[tag][node.type]:joCollect.view[tag];var view=new o(args);}
if(node.id)
ui[node.id]=view;return view;}
return ui;}};joView=function(data){this.changeEvent=new joSubject(this);this.setContainer();if(data)
