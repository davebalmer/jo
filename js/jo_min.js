...
this.moved=true;},onOut:function(e){if(!this.inMotion)
return;if(e.clientX>=0&&e.clientX<=this.container.offsetWidth&&e.clientY>=0&&e.clientX<=this.container.offsetHeight){return;}
else{joEvent.stop(e);this.onUp(e);this.reset();}},onUp:function(e){if(!this.inMotion)
return;this.inMotion=false;var end=this.getMouse(e);var node=this.container.firstChild;var top=node.offsetTop;joEvent.stop(e);var dy=0;for(var i=0;i<this.points.length-1;i++)
dy+=(this.points[i].y-this.points[i+1].y);var max=0-node.offsetHeight+this.container.offsetHeight-this.bump;if(Math.abs(dy)>5&&!this.quickSnap){var flick=dy*(this.velocity*(node.offsetHeight/this.container.offsetHeight));if(flick+top<max||flick+top>0){console.log('flickfast');joDOM.addCSSClass(node,"flickfast");}
else{console.log('flick');joDOM.addCSSClass(node,"flick");}
this.eventset=joEvent.on(node,"webkitTransitionEnd",this.snapBack,this);this.scrollBy(flick,false);}
...
this.switchScroller();joStack.prototype.pop.call(this);}});joToolbar=function(data){joContainer.apply(this,arguments);};joToolbar.extend(joContainer,{tagName:"jotoolbar"});joScreen=function(data){this.resizeEvent=new joSubject(this);this.menuEvent=new joSubject(this);this.activateEvent=new joSubject(this);this.deactivateEvent=new joSubject(this);this.backEvent=new joSubject(this);this.forwardEvent=new joSubject(this);joContainer.apply(this,arguments);};joScreen.extend(joContainer,{setupEvents:function(){joEvent.on(window,"resize",this.resizeEvent.fire,this);joEvent.on(window,"appmenushow",this.menuEvent.fire,this);joEvent.on(window,"activate",this.activateEvent.fire,this);joEvent.on(window,"deactivate",this.deactivateEvent.fire,this);joEvent.on(window,"back",this.backEvent.fire,this);},createContainer:function(){return document.body;},showPopup:function(data){if(!this.popup){this.popup=new joShim(new joPopup(data));}
else{this.popup.setData(data);}
this.popup.attach(this);this.popup.activate();},hidePopup:function(){if(this.popup)
